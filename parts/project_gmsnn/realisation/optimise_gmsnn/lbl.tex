...
Cet algorithme est une implémentation naïve d'un entraînement couche par couche appliqué à l'architecture \gls{gmsnn}. Cette algorithme s'apparente aux algorithmes IM \foreign{(Information-Maximizing)}. %TODO \autocite{hm}.
...
Le fonctionnement détaillé de l'algorithme est disponible dans l'annexe \ref{subsec:lbl} (\autopageref{subsec:lbl}).
...
Les performances de l'algorithme sont disponible dans le rapport dans l'annexe \ref{subsec:test_perf} (\autopageref{subsec:lbl}).
