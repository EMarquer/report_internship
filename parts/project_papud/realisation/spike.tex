...
Il à donc été décidé, comme décrit \autoref{hex} (\autopageref{hex}), d'intégrer au prétraitement des données la gestion de ces codes.
Cette intégration à cependant dù attendre la fin de la réalisation de la nouvelle version du gestionnaire de données, décrite \autoref{sec:papud_mulitiqueue} (\autopageref{sec:papud_mulitiqueue}).

Le déroulement de l'analyse et les conclusions détaillées sont disponibles dans l'annexe \ref{anx:spike} (\autopageref{anx:spike}).
