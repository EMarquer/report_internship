...
\subsubsection{\Glsentrytext{parameter}} \label{def:parameter}
...
\subsubsection{Principales \glsentryplural{architecture} de \glsentryplural{nn}} \label{def:lstm} \label{def:rnn}
...
\begin{itemize}
	\item Le \gls{nn} intégralement connecté ou module linéaire est un des types de \glspl{nn} les plus simple, qui est emblématique des \glspl{nn}.
	...
	\item Le \gls{lstm} est un \gls{rnn} particulier équipé d'une mémoire supplémentaire, plus puissant mais plus coûteux à entraîner qu'un \gls{rnn} classique.
\end{itemize}
...
\subsubsection{Traitement automatique des langues (\glsentrytext{nlp})} \label{subsec:nlp}\label{def:nlp}
...
utilisée pour estimer la probabilité d'apparition du prochain mot ou caractère, d'après Wikipédia \autocite{wiki_lm}.
...
Parmi les information contenues dans le contexte d'un mot, on peut trouver aussi bien le sens des mots environnants que la structure syntaxique de la phrase, ou encore des informations plus générales comme le nom du dernier roi de France.
...
On considère que, plus on a d'informations contextuelles, plus le \gls{lm} est précis. Par exemple, si on nous dit \og un chat \fg{}, il sera plus difficile de prédire la couleur du chat que si on nous dit \og un chat noir \fg{}.
